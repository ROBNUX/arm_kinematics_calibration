...
"""
def plot_segment(figure, plot_times, boundaries, position, velocity, acceleration, jerk, j_max=None,
    ...
    #for joint_i in range(position.shape[1]):
    c = joint_colors[1]
    axes[0].plot(plot_times, position, c=c)
    axes[0].set_ylabel('position')
    axes[1].plot(plot_times, velocity, c=c)
    axes[1].set_ylabel('velocity')
    axes[2].plot(plot_times, acceleration, c=c)
    axes[2].set_ylabel('acceleration')
    axes[3].plot(plot_times, jerk, c=c)
    axes[3].set_ylabel('jerk')

    axes[0].vlines(boundaries, position.min(), position.max(), color=(0.8, 0.8, 0.8))
    ...
    plt.legend()
    plt.suptitle("scurve_segment")




def fit_and_plot_segment(npoints, p_start, p_end, v_start, v_end, p_max, v_max, a_max, j_max):
    '''
    ...
    '''
    plot_times= np.zeros((npoints,1), dtype=float)
    position= np.zeros((npoints,1), dtype=float)
